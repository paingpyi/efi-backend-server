            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-tools">
                        @include('admin.blocks.aboutefil.menu')
                    </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                                ...
                                @foreach ($blocks as $block)
                                    <tr>
                                        <td class="text-nowrap"><a
                                                href="{{ route('edit#efil#block', Illuminate\Support\Facades\Crypt::encryptString($block->id)) }}">{{ json_decode($block->title, true)['en-us'] }}
                                            </a></td>
                                        <td>{!!json_decode($block->description, true)['en-us'] !!}</td>
                                        <td class="text-nowrap">
                                            <form
                                                action="{{ route('deactivate#efil#block', Illuminate\Support\Facades\Crypt::encryptString($block->id)) }}"
                                                method="post">
                                                @csrf
                                                <div class="btn-group">
                                                    <button type="submit"
    ...
    : '<i class="fas fa-square"></i> No' !!}</button>
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                            href="{{ route('edit#efil#block', Illuminate\Support\Facades\Crypt::encryptString($block->id)) }}">Edit</a>
                                                        <button type="submit"
