                                ...
                                @foreach ($newsroom as $news)
                                    <tr>
                                        <td class="text-nowrap"><a
                                                href="{{ route('edit#news', Illuminate\Support\Facades\Crypt::encryptString($news->id)) }}">{{ $news->title }}<br><span
                                                    class="badge badge-info">{{ $news->title_burmese }}</span>
                                            </a></td>
                                            ...
                                            <div class="btn-group">
                                                @if ($news->status == 'published')
                                                    <button id="btn-publish" type="type" class="btn btn-success text-capitalize"><i
                                                            class="fas fa-check-square"></i> {{ $news->status }}</button>
                                                    <button type="button"
                                                        class="btn btn-success dropdown-toggle dropdown-toggle-split"
                                                        data-toggle="dropdown" aria-expanded="false">
                                                        <span class="sr-only">Toggle Dropdown</span>
                                                    </button>
                                                @elseif($news->status == 'unpublished')
                                                    <button id="btn-publish" type="type" class="btn btn-danger text-capitalize"><i
                                                            class="fas fa-square"></i> {{ $news->status }}</button>
                                                    <button type="button"
                                                ...
                                                @else
                                                    <button id="btn-publish" type="type" class="btn btn-warning text-capitalize"><i
                                                            class="fas fa-file"></i> {{ $news->status }}</button>
                                                    <button type="button"
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item"
                                                        href="{{ route('edit#news', Illuminate\Support\Facades\Crypt::encryptString($news->id)) }}">Edit</a>
                                                    <div class="dropdown-divider"></div>
                                                    <form id="publish-form"
                                                        action="{{ route('unpublishing#news', Illuminate\Support\Facades\Crypt::encryptString($news->id)) }}"
                                                        method="post">
                                                        @csrf
                                                        <button type="submit"
                                                            class="dropdown-item">{{ $news->status == 'published' ? 'Unpublish' : 'Publish' }}</button>
                                                    </form>
                                                    @if ($news->status == 'published')
                                                        <form
                                                            action="{{ route('drafting#news', Illuminate\Support\Facades\Crypt::encryptString($news->id)) }}"
                                                            method="post">
                                                            @csrf
