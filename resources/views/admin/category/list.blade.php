                        ...
                        <table id="data-table" class="table table-bordered table-striped table-hover w-100">
                            ...
                            <tbody>
                                @foreach ($categories as $category)
                                    <tr>
                                        <td>{{ $category->name }}<br><span
                                                class="badge badge-info">{{ $category->name_burmese }}</span></td>
                                        <td>
                                        ...
                                        <td class="text-nowrap">
                                            {!! $category->description !!}<br><span
                                            class="badge badge-info">{!! $category->description_burmese !!}</span>
                                        </td>
                                        <td class="text-nowrap">
                                            <form
                                                action="{{ route('deactivate#category', Illuminate\Support\Facades\Crypt::encryptString($category->id)) }}"
                                                method="post">
                                                @csrf
                                                <div class="btn-group">
                                                    <button type="submit"
                                                        class="btn {{ $category->is_active ? 'btn-success' : 'btn-danger' }}">{!! $category->is_active
    ? '<i
                                                    class="fas fa-check-square"></i> Active'
    : '<i class="fas fa-square"></i> Deactivated' !!}</button>
                                                    <button type="button"
                                                        class="btn {{ $category->is_active ? 'btn-success' : 'btn-danger' }} dropdown-toggle dropdown-toggle-split"
                                                        data-toggle="dropdown" aria-expanded="false">
                                                        <span class="sr-only">Toggle Dropdown</span>
                                                    </button>
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                            href="{{ route('edit#category', Illuminate\Support\Facades\Crypt::encryptString($category->id)) }}">Edit</a>
                                                        <button type="submit"
                                                            class="dropdown-item">{{ $category->is_active ? 'Deactivate' : 'Activate' }}</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </td>
