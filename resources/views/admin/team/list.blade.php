                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if (isset($team->permissions))
                                                @foreach (json_decode($team->permissions) as $permission)
                                                    <div class="d-inline p-2 m-2 text-capitalize"><i
                                                            class="fas fa-cubes"></i>&nbsp;{{ Str::replace('#', ' ', $permission) }}
                                                    </div>
                                                @endforeach
                                            @endif
                                        </td>
                                        <td class="text-nowrap">{{ $team->user_name }}</td>
                                        <td class="text-nowrap">
                                            <form
                                                action="{{ route('deactivate#team', Illuminate\Support\Facades\Crypt::encryptString($team->id)) }}"
                                                method="post">
                                                @csrf
                                                <div class="btn-group">
                                                    <button type="submit"
                                                        class="btn {{ $team->is_active ? 'btn-success' : 'btn-danger' }}">{!! $team->is_active
    ? '<i
                                                class="fas fa-check-square"></i> Active'
    : '<i class="fas fa-square"></i> Deactivated' !!}</button>
                                                    <button type="button"
                                                        class="btn {{ $team->is_active ? 'btn-success' : 'btn-danger' }} dropdown-toggle dropdown-toggle-split"
                                                        data-toggle="dropdown" aria-expanded="false">
                                                        <span class="sr-only">Toggle Dropdown</span>
                                                    </button>
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                            href="{{ route('edit#team', Illuminate\Support\Facades\Crypt::encryptString($team->id)) }}">Edit</a>
                                                        <button type="submit"
                                                            class="dropdown-item">{{ $team->is_active ? 'Deactivate' : 'Activate' }}</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </td>
